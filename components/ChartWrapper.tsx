...
export default function ChartWrapper({ data, layout = {}, config = {}, className }: ChartWrapperProps) {
  ...
  useEffect(() => {
    ...
    const defaultLayout = {
      ...
      xaxis: { showgrid: true, gridcolor: 'rgba(255,255,255,0.05)', zeroline: false, automargin: true },
      yaxis: { showgrid: true, gridcolor: 'rgba(255,255,255,0.05)', zeroline: false, automargin: true },
      legend: { orientation: 'h', x: 0, y: 0.1, bgcolor: 'rgba(22,27,34,0.6)', borderwidth: 0, font: { size: 11, color: '#c9d1d9' } },
      autosize: true,
      ...layout, // merge user layout (page-provided)
    };

    // Default config
    const defaultConfig = { displayModeBar: false, responsive: true, scrollZoom: true, ...config };
  ...
  if (!Plotly) {
    return (
      <div className={`${className} bg-dark-card border border-dark-border rounded-lg flex items-center justify-center`} style={{ minHeight: 400 }}>
